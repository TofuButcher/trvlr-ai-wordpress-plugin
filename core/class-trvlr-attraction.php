<?php

/**
 * Attraction CPT and taxonomy registration.
 *
 * @package    Trvlr
 * @subpackage Trvlr/core
 */
class Trvlr_Attraction
{
	public function __construct() {}

	/**
	 * Register the `trvlr_attraction` custom post type.
	 *
	 * @return void
	 */
	public function register_post_type()
	{
		if (function_exists('trvlr_is_attraction_post_type_disabled') && trvlr_is_attraction_post_type_disabled()) {
			return;
		}

		$labels = array(
			'name'                  => _x('Traveloris Attractions', 'Post Type General Name', 'trvlr'),
			'singular_name'         => _x('Traveloris Attraction', 'Post Type Singular Name', 'trvlr'),
			'menu_name'             => __('Traveloris Attractions', 'trvlr'),
			'name_admin_bar'        => __('Traveloris Attraction', 'trvlr'),
			'archives'              => __('Traveloris Attraction Archives', 'trvlr'),
			'attributes'            => __('Traveloris Attraction Attributes', 'trvlr'),
			'parent_item_colon'     => __('Parent Traveloris Attraction:', 'trvlr'),
			'all_items'             => __('All Traveloris Attractions', 'trvlr'),
			'add_new_item'          => __('Add New Traveloris Attraction', 'trvlr'),
			'add_new'               => __('Add New', 'trvlr'),
			'new_item'              => __('New Traveloris Attraction', 'trvlr'),
			'edit_item'             => __('Edit Traveloris Attraction', 'trvlr'),
			'update_item'           => __('Update Traveloris Attraction', 'trvlr'),
			'view_item'             => __('View Traveloris Attraction', 'trvlr'),
			'view_items'            => __('View Traveloris Attractions', 'trvlr'),
			'search_items'          => __('Search Traveloris Attractions', 'trvlr'),
			'not_found'             => __('Not found', 'trvlr'),
			'not_found_in_trash'    => __('Not found in Trash', 'trvlr'),
			'featured_image'        => __('Featured Image', 'trvlr'),
			'set_featured_image'    => __('Set featured image', 'trvlr'),
			'remove_featured_image' => __('Remove featured image', 'trvlr'),
			'use_featured_image'    => __('Use as featured image', 'trvlr'),
			'insert_into_item'      => __('Insert into Traveloris attraction', 'trvlr'),
			'uploaded_to_this_item' => __('Uploaded to this Traveloris attraction', 'trvlr'),
			'items_list'            => __('Traveloris Attractions list', 'trvlr'),
			'items_list_navigation' => __('Traveloris Attractions list navigation', 'trvlr'),
			'filter_items_list'     => __('Filter Traveloris attractions list', 'trvlr'),
		);
		$args = array(
			'label'                 => __('Traveloris Attraction', 'trvlr'),
			'description'           => __('Tours and Experiences from Traveloris', 'trvlr'),
			'labels'                => $labels,
			'supports'              => array('title', 'editor', 'thumbnail', 'custom-fields', 'excerpt'),
			'taxonomies'            => array('trvlr_attraction_tag', 'category', 'post_tag'),
			'hierarchical'          => false,
			'public'                => true,
			'show_ui'               => true,
			'show_in_menu'          => true,
			'menu_position'         => 8,
			'menu_icon'             => 'dashicons-location',
			'show_in_admin_bar'     => true,
			'show_in_nav_menus'     => true,
			'can_export'            => true,
			'has_archive'           => true,
			'exclude_from_search'   => false,
			'publicly_queryable'    => true,
			'capability_type'       => 'post',
			'show_in_rest'          => true,
		);
		$args = apply_filters( 'trvlr_attraction_args', $args );
		register_post_type('trvlr_attraction', $args);
	}

	/**
	 * Register the `trvlr_attraction_tag` taxonomy.
	 *
	 * @return void
	 */
	public function register_taxonomy()
	{
		if (function_exists('trvlr_is_attraction_post_type_disabled') && trvlr_is_attraction_post_type_disabled()) {
			return;
		}

		$labels = array(
			'name'                       => _x('Attraction Tags', 'Taxonomy General Name', 'trvlr'),
			'singular_name'              => _x('Attraction Tag', 'Taxonomy Singular Name', 'trvlr'),
			'menu_name'                  => __('Attraction Tags', 'trvlr'),
			'all_items'                  => __('All Tags', 'trvlr'),
			'parent_item'                => __('Parent Tag', 'trvlr'),
			'parent_item_colon'          => __('Parent Tag:', 'trvlr'),
			'new_item_name'              => __('New Tag Name', 'trvlr'),
			'add_new_item'               => __('Add New Tag', 'trvlr'),
			'edit_item'                  => __('Edit Tag', 'trvlr'),
			'update_item'                => __('Update Tag', 'trvlr'),
			'view_item'                  => __('View Tag', 'trvlr'),
			'separate_items_with_commas' => __('Separate tags with commas', 'trvlr'),
			'add_or_remove_items'        => __('Add or remove tags', 'trvlr'),
			'choose_from_most_used'      => __('Choose from the most used', 'trvlr'),
			'popular_items'              => __('Popular Tags', 'trvlr'),
			'search_items'               => __('Search Tags', 'trvlr'),
			'not_found'                  => __('Not Found', 'trvlr'),
			'no_terms'                   => __('No tags', 'trvlr'),
			'items_list'                 => __('Tags list', 'trvlr'),
			'items_list_navigation'      => __('Tags list navigation', 'trvlr'),
		);
		$args = array(
			'labels'                     => $labels,
			'hierarchical'               => false,
			'public'                     => true,
			'show_ui'                    => true,
			'show_admin_column'          => true,
			'show_in_nav_menus'          => true,
			'show_tagcloud'              => true,
			'show_in_rest'               => true,
		);
		register_taxonomy('trvlr_attraction_tag', array('trvlr_attraction'), $args);
	}
}
